...
public class SimpleLOD_MaterialPopup : EditorWindow
{
        ...
        for (int order = 0; order < submeshRows.Count * 2; order += 2)
        {
            for (int i = 0; i < submeshRows.Count; i++)
            {
                ...
                if ((int)row["order"] == order)
                {
                    ...
                    {
                        if (dragFromPosition.y >= ly && dragFromPosition.y <= ly + lh + m)
                        {  // row under cursor
                            ...
                            { // is readable
                                ...
                                if (j >= submeshRows.Count)
                                { // has no other rows merged into this
                                    Texture atlas = null;
            ...
            if (dragEnded)
            {
                if (draggedEnough)
                {
                    ...
                    if (dragIntoRow != null)
                    {
                        ...
                        if (dragIntoRow["atlasMat"] == dragIntoRow["origMat"])
                        { // doesnt have an atlas yet
            ...
            if (dragEnded)
            {
                if (draggedEnough)
                {
                    ...
                    if (dragIntoAtlas != null)
                    {
                        ...
                        if (dragIntoAtlas["atlasMat"] == dragIntoAtlas["origMat"])
                        { // doesnt have an atlas yet
    ...
    private void CreateAtlas(Hashtable aRow)
    {
        ...
        if (texturesToInclude.Count <= keys.Count)
        { // Only 1 row included in atlas
            for (int j = 0; j < atlasRows.Count; j++)
            {
