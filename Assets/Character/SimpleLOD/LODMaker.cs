...
public static class LODMaker
{
    ...
    public static void MakeLODMesh(Mesh orig, SkinnedMeshRenderer smr, float aMaxWeight)
    {
        MakeLODMesh(orig, smr, true, aMaxWeight, 1f, 1f, 1f, 1f, 1f, true);
    }
    public static void MakeLODMesh(Mesh orig, MeshFilter mf, float aMaxWeight)
    {
        MakeLODMesh(orig, mf, false, aMaxWeight, 1f, 1f, 1f, 1f, 1f, true);
    }
    public static void MakeLODMesh(Mesh orig, object smr, bool skinnedmesh, float aMaxWeight, float removeSmallParts, float protectNormals, float protectUvs, float smallTrianglesFirst, float protectSubMeshesAndSharpEdges, bool recalcNormals)
    {
        ...
        {   // read triangles of submeshes 1 by 1 because I dont know the internal order of the mesh
        ...
        Loom.RunAsync(() =>
        {
            ...
            MakeLODMesh(vs, ns, uv1s, uv2s, uv3s, uv4s, colors32, ts, ref bindposes, bws, ref subMeshOffsets, meshBounds, aMaxWeight, removeSmallParts, sideLengthWeight, oldAngleWeight, newAngleWeight, uvWeight, areaDiffWeight, normalWeight, vertexWeight, centerDistanceWeight, out newVs, out newNs, out newUv1s, out newUv2s, out newUv3s, out newUv4s, out newColors32, out newTs, out newBws);

            UnityEngine.Debug.Log("compression:" + aMaxWeight + ", vertices:" + vs.Length + " -> " + newVs.Count + ", triangles:" + (ts.Length / 3) + " -> " + (newTs.Count / 3) + "\n" + startTimeString + "\nended " + DateTime.Now.ToString("yyy/MM/dd hh:mm:ss.fff"));

            Loom.QueueOnMainThread(() =>
            {
                #region new mesh
                Mesh newMesh = new Mesh();
                newMesh.vertices = newVs.ToArray();
                if (newNs != null && newNs.Count > 0) newMesh.normals = newNs.ToArray();
                if (newUv1s != null && newUv1s.Count > 0) newMesh.uv = newUv1s.ToArray();
                if (newUv2s != null && newUv2s.Count > 0) newMesh.uv2 = newUv2s.ToArray();
                if (newUv3s != null && newUv3s.Count > 0) newMesh.uv3 = newUv3s.ToArray();
                if (newUv4s != null && newUv4s.Count > 0) newMesh.uv4 = newUv4s.ToArray();
                if (newColors32 != null && newColors32.Count > 0) newMesh.colors32 = newColors32.ToArray();
                if (newBws != null && newBws.Count > 0) newMesh.boneWeights = newBws.ToArray();
                if (bindposes != null && bindposes.Length > 0) newMesh.bindposes = bindposes;
                if (subMeshOffsets.Length == 1)
                {
                    newMesh.triangles = newTs.ToArray();
                }
                else
                {
                    newMesh.subMeshCount = subMeshOffsets.Length;
                    for (int s = 0; s < subMeshOffsets.Length; s++)
                    {
                        subMeshOffsets[s] = Mathf.Max(0, subMeshOffsets[s]);
                        int end = s + 1 < subMeshOffsets.Length ? subMeshOffsets[s + 1] : newTs.ToArray().Length;
                        if (end - subMeshOffsets[s] > 0)
                        {
                            int[] subTs = new int[end - subMeshOffsets[s]];
                            Array.Copy(newTs.ToArray(), subMeshOffsets[s], subTs, 0, end - subMeshOffsets[s]);
                            newMesh.SetTriangles(subTs, s);
                        }
                        else
                        {
                            newMesh.SetTriangles((int[])null, s);
                        }
                    }
                }
                if (recalcNormals || newMesh.normals == null || newMesh.normals.Length <= 0) newMesh.RecalculateNormals();
                newMesh.RecalculateTangents();
                #endregion

                if (skinnedmesh)
                {
                    SkinnedMeshRenderer s = smr as SkinnedMeshRenderer;
                    s.sharedMesh = newMesh;
                }
                else
                {
                    MeshFilter mf = smr as MeshFilter;
