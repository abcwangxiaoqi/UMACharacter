...
namespace UMAEditor
{
	...
	{
	    public static bool ConvertDefaultAssets_DiffuseAlpha_Normal_Specular(string diffuseAlpha, string normal, string specular, string assetName, string textureFolder, Shader shader)
	    {
	       
	        var diffuseName = textureFolder +"/"+ assetName + "_diffuse.png";
            diffuseName = diffuseName.Replace("//", "/");
	        if (!CopyTextureToPng(diffuseAlpha, diffuseName)) return false;

	        var normalName = textureFolder  +"/"+ assetName + "_normal.png";
            normalName = normalName.Replace("//", "/");
	        if (!CopyTextureToPng(normal, normalName)) return false;

	        var specularName = textureFolder  +"/"+ assetName + "_specular.png";
            specularName = specularName.Replace("//", "/");
